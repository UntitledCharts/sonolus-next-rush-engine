...
def begin_particle_chunk(particle: Particle) -> float:
    ...
    while removed:
        ...
        for key, entry in ParticleHandler.entries.items():
            if entry.particle_id == particle_id and entry.chunk_id == chunk:
                entry.particle.destroy()
...
def emit_particle(
    ...
    if not managed:
        ...
        return
    particle_id = particle.id
    if manage_kind in (ParticleManageKind.LANE, ParticleManageKind.FLICK):
    ...
    ParticleHandler.entries[key] = ParticleEntry(
        particle=handle, spawn_time=time(), chunk_id=chunk, particle_id=particle_id
    )
