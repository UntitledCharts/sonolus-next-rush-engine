...
def handle_note_particles(
    ...
    is_flick = effect_kind in {NoteEffectKind.NORM_FLICK, NoteEffectKind.CRIT_FLICK}
    flick_slot = floor(lane) + 0.5
        ...
        if linear_particle.is_available:
            ...
            if linear_particle == particles.linear_good:
                ...
                emit_particle(
                    ...
                    0.5 / speed,
                    ParticleManageKind.FLICK if is_flick else ParticleManageKind.REST,
                    flick_slot,
                    chunk,
                    managed,
                )
        ...
        if particles.lane.is_available:
            ...
            if particles.lane.id == BaseParticles.critical_flick_note_lane_linear.id:
                ...
                for slot_lane in iter_slot_lanes(lane, size):
                    emit_particle(
                        particles.lane,
                        layout_lane(slot_lane, 0.5, y_offset=lane_y_offset),
                        1 / speed,
...
def _emit_critical_flick_lane(particle, center_lane: float, size: float, y_offset: float, chunk: float, managed: bool):
    ...
    if min_i < -127:
        ...
        emit_particle(
            particle,
            layout_lane(merged_center, merged_size, y_offset=y_offset),
            1 / speed,
            ParticleManageKind.LANE,
            -127.0,
            chunk,
    ...
    for i in range(start_i, end_i + 1):
        ...
        emit_particle(
            particle,
            layout_lane(slot_lane, 0.5, y_offset=y_offset),
            1 / speed,
            ParticleManageKind.LANE,
            slot_lane,
            chunk,
